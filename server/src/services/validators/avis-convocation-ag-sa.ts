...
export function validateAvisConvocationAgSa(data: Record<string, unknown>): ValidationError[] {
  ...
  if (!(data.type_ag as string)?.trim()) {
    errors.push({ field: "type_ag", message: "Le type d'assemblée est obligatoire (ordinaire, extraordinaire ou mixte)" });
  }
