...
export function validatePouvoirAgSa(data: Record<string, unknown>): ValidationError[] {
  ...
  if (!(data.heure_ag as string)?.trim()) {
    errors.push({ field: "heure_ag", message: "L'heure de l'assemblée générale est obligatoire" });
  }
