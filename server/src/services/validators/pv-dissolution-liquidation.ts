...
export function validatePvDissolutionLiquidation(data: Record<string, unknown>): ValidationError[] {
  ...
  if (!(data.date_dissolution as string)?.trim()) {
    errors.push({ field: "date_dissolution", message: "La date de dissolution est obligatoire" });
  }
