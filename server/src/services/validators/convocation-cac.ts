...
export function validateConvocationCac(data: Record<string, unknown>): ValidationError[] {
  ...
  if (!(data.type_reunion as string)?.trim()) {
    errors.push({ field: "type_reunion", message: "Le type de réunion est obligatoire" });
  }
