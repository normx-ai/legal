...
export function validateProjetFusionAbsorbeeAbsorbante(data: Record<string, unknown>): ValidationError[] {
  ...
  const actions_detenues = data.actions_detenues_par_a as number;
  if (!actions_detenues || actions_detenues <= 0) {
    errors.push({ field: "actions_detenues_par_a", message: "Le nombre d'actions d\u00e9tenues par A dans B est obligatoire" });
  }
