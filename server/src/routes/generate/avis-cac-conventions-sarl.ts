...
avisCacConventionsSarlRoute.post("/avis-cac-conventions-sarl", requireAuth(), async (req: AuthRequest, res: Response) => {
  try {
    const errors = validateAvisCacConventionsSarl(req.body);
    if (errors.length > 0) {
      return res.status(400).json({ errors });
    }

    const templateData = prepareAvisCacConventionsSarlData(req.body);
    const docxBuffer = generateDocx("avis-cac-conventions-sarl/avis.docx", templateData);
    ...
    const filename = `avis-cac-conventions-sarl-${safeNom}-${timestamp}.docx`;
