...
projetFusionAbsorbeeAbsorbanteRoute.post("/projet-fusion-absorbee-absorbante", requireAuth(), async (req: AuthRequest, res: Response) => {
  try {
    const errors = validateProjetFusionAbsorbeeAbsorbante(req.body);
    if (errors.length > 0) {
      return res.status(400).json({ errors });
    }

    const templateData = prepareProjetFusionAbsorbeeAbsorbanteData(req.body);
    const docxBuffer = generateDocx("projet-fusion-absorbee-absorbante/projet-fusion-absorbee-absorbante.docx", templateData);
    ...
    const filename = `projet-fusion-absorbee-absorbante-${safeNom}-${timestamp}.docx`;
    ...
    console.error("[generate/projet-fusion-absorbee-absorbante]", err);
