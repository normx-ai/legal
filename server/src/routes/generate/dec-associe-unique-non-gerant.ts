...
decAssocieUniqueNonGerantRoute.post("/dec-associe-unique-non-gerant", requireAuth(), async (req: AuthRequest, res: Response) => {
  try {
    const errors = validateDecAssocieUniqueNonGerant(req.body);
    if (errors.length > 0) {
      return res.status(400).json({ errors });
    }

    const templateData = prepareDecAssocieUniqueNonGerantData(req.body);
    const docxBuffer = generateDocx("dec-associe-unique-non-gerant/decisions.docx", templateData);
    ...
    const filename = `dec-associe-unique-non-gerant-${safeNom}-${timestamp}.docx`;
