...
interface DecActionnaireUniqueNonAGState {
  denomination: string;
  siege_social: string;
  capital: string;
  ...
  currentStep: number;
  set: (data: Partial<DecActionnaireUniqueNonAGState>) => void;
...
const INITIAL: Omit<DecActionnaireUniqueNonAGState, "set" | "toggleDecision" | "nextStep" | "prevStep" | "reset"> = {
  denomination: "",
  siege_social: "",
  capital: "",
  forme_juridique: "SA",
...
const useStore = create<DecActionnaireUniqueNonAGState>((set) => ({
  ...INITIAL,
...
export default function DecActionnaireUniqueNonAGWizardScreen() {
  const { colors } = useTheme();
  const w = useStore();
  const { isGenerating, generatedUrl, error, generate } = useDocumentGeneration("/generate/dec-actionnaire-unique-non-ag", w.nextStep);
  ...
  const previewLines = useMemo(() => {
    ...
    const lines: any[] = [
      ...
      { text: "(Hors Assemblée Générale)", center: true, size: "md" as const },
      { text: "━━━━━━━━━━━━━━━━━━━━━━", center: true },
  ...
  return (
    <WizardLayout
      title="Décisions actionnaire unique hors AG (SA/SASU)"
      steps={STEPS}
        ...
        {w.currentStep === 2 && (
          <>
            <Text style={{ fontFamily: fonts.regular, fontSize: 13, color: colors.textSecondary, marginBottom: 16 }}>
              Sélectionnez les décisions prises par l'actionnaire unique (hors AG).
            </Text>
            ...
            <>
              <View style={{ backgroundColor: "#ffffff", padding: 32, marginBottom: 20, borderWidth: 1, borderColor: colors.border }}>
                <Text style={{ fontFamily: fonts.heading, fontWeight: fontWeights.heading, fontSize: 20, color: "#1f2937", textAlign: "center", marginBottom: 16 }}>
                  DÉCISIONS DE L'ACTIONNAIRE UNIQUE (HORS AG)
                </Text>
