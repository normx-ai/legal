...
interface ConvAgoState {
  ...
  lieu_envoi: string;
  currentStep: number;
  set: (data: Partial<ConvAgoState>) => void;
  nextStep: () => void;
  prevStep: () => void;
  reset: () => void;
}
...
const useStore = create<ConvAgoState>((set) => ({
  ...initialState,
...
export default function ConvAgoWizardScreen() {
  ...
  const previewLines = useMemo(() => {
    ...
    const lines: PreviewLine[] = [
      ...
      { text: `Vous êtes convoqué(e) à l'Assemblée Générale Ordinaire qui se tiendra le ${v(w.date_ag)} à ${v(w.heure_ag)}, au siège social, ${v(w.lieu_ag)}.`, spaceBefore: true },
      { text: "", spaceBefore: true },
  ...
  return (
    ...
    >
      ...
      {w.currentStep === 2 && (
        <>
          <Field colors={colors} label="Date de l'AGO" value={w.date_ag} onChangeText={(v) => w.set({ date_ag: v })} placeholder="Ex: 25 mars 2026" />
