...
interface PouvoirAgSaState {
  ...
  currentStep: number;
  set: (data: Partial<PouvoirAgSaState>) => void;
  nextStep: () => void;
  prevStep: () => void;
  reset: () => void;
}
...
const useStore = create<PouvoirAgSaState>((set) => ({
  denomination: "",
  forme_juridique: "SA",
  siege_social: "",
  capital: "",
...
export default function PouvoirAgSaWizardScreen() {
  const { colors } = useTheme();
  const w = useStore();
  const { isGenerating, generatedUrl, error, generate } = useDocumentGeneration("/generate/pouvoir-ag-sa", w.nextStep);
  const handleGenerate = () => generate({
        ...
        mandant_nombre_actions: w.mandant_nombre_actions,
  ...
  const previewLines = useMemo(() => {
    ...
    const lines: PreviewLine[] = [
      ...
      { text: "━━━━━━━━━━━━━━━━━━━━━━", center: true },
      { text: "POUVOIR", bold: true, center: true, size: "lg" as const },
      { text: `Pour l'${v(w.type_ag)} du ${v(w.date_ag)}`, center: true, size: "md" as const },
      { text: "━━━━━━━━━━━━━━━━━━━━━━", center: true },
      { text: "", spaceBefore: true },
      { text: `Je soussigné(e), ${w.mandant_civilite} ${v(w.mandant_prenom)} ${v(w.mandant_nom)}, demeurant à ${v(w.mandant_adresse)}, actionnaire détenant ${w.mandant_nombre_actions || "..."} actions,`, spaceBefore: true },
      { text: "", spaceBefore: true },
      ...
      { text: "", spaceBefore: true },
      { text: `de me représenter à l'${v(w.type_ag)} qui se tiendra le ${v(w.date_ag)} à ${v(w.heure_ag)}, à ${v(w.lieu_ag)}.`, spaceBefore: true },
      { text: "", spaceBefore: true },
    ...
    return lines.filter(l => l.text !== undefined);
      ...
      w.mandant_civilite, w.mandant_nom, w.mandant_prenom, w.mandant_adresse, w.mandant_nombre_actions,
      w.mandataire_civilite, w.mandataire_nom, w.mandataire_prenom, w.mandataire_adresse,
      w.type_ag, w.date_ag, w.heure_ag, w.lieu_ag,
      w.lieu_signature, w.date_signature]);

  return (
    <WizardLayout
      title="Pouvoir pour AG (SA)"
      steps={STEPS}
    ...
    >
        ...
        {w.currentStep === 1 && (
          <>
            ...
            <Field colors={colors} label="Nombre d'actions détenues" value={w.mandant_nombre_actions ? String(w.mandant_nombre_actions) : ""} onChangeText={(v) => w.set({ mandant_nombre_actions: parseInt(v) || 0 })} keyboardType="numeric" />
          </>
        )}
          ...
          <>
            <View style={{ backgroundColor: "#ffffff", padding: 32, marginBottom: 20, borderWidth: 1, borderColor: colors.border }}>
              ...
              <Text style={{ fontFamily: fonts.regular, fontSize: 13, color: "#374151", marginBottom: 8 }}>
                Je soussigné(e), {w.mandant_civilite} {w.mandant_prenom} {w.mandant_nom}, demeurant à {w.mandant_adresse}, actionnaire détenant {w.mandant_nombre_actions} actions, donne pouvoir à {w.mandataire_civilite} {w.mandataire_prenom} {w.mandataire_nom}, demeurant à {w.mandataire_adresse}, de me représenter à l'{w.type_ag}.
              </Text>
