...
interface Associe {
  ...
  nationalite: string; profession: string; adresse: string; apport: number; type_apport: string;
}
...
interface SncState {
  ...
  currentStep: number;
  set: (data: Partial<SncState>) => void;
  nextStep: () => void; prevStep: () => void; reset: () => void;
  addAssocie: () => void; removeAssocie: (i: number) => void; updateAssocie: (i: number, data: Partial<Associe>) => void;
}

const emptyAssocie = (): Associe => ({ civilite: "Monsieur", nom: "", prenom: "", date_naissance: "", lieu_naissance: "", nationalite: "congolaise", profession: "", adresse: "", apport: 0, type_apport: "numeraire" });

const INITIAL_ASSOC = [emptyAssocie(), emptyAssocie()];

const useStore = create<SncState>((set) => ({
  ...
  reset: () => set({ denomination: "", objet_social: "", siege_social: "", ville: "Brazzaville", pays: "R\u00e9publique du Congo", duree: 99, capital: 0, valeur_nominale: 10000, associes: [emptyAssocie(), emptyAssocie()], gerant_civilite: "Monsieur", gerant_nom: "", gerant_prenom: "", gerant_date_naissance: "", gerant_lieu_naissance: "", gerant_nationalite: "congolaise", gerant_adresse: "", gerant_duree_mandat: "dur\u00e9e de la soci\u00e9t\u00e9", lieu_signature: "Brazzaville", date_signature: "", currentStep: 0 }),
  addAssocie: () => set((s) => ({ associes: [...s.associes, emptyAssocie()] })),
  removeAssocie: (i) => set((s) => ({ associes: s.associes.filter((_, idx) => idx !== i) })),
  updateAssocie: (i, data) => set((s) => ({ associes: s.associes.map((a, idx) => idx === i ? { ...a, ...data } : a) })),
}));

const STEPS = ["Soci\u00e9t\u00e9", "Associ\u00e9s", "Capital", "G\u00e9rance", "R\u00e9capitulatif", "Aper\u00e7u"];

export default function SncWizardScreen() {
  const { colors } = useTheme();
  const w = useStore();
  const { isGenerating, generatedUrl, error, generate } = useDocumentGeneration("/generate/snc", w.nextStep);
  ...
  const previewLines = useMemo(() => {
    ...
    return [
      ...
      { text: "Soci\u00e9t\u00e9 en Nom Collectif", center: true, size: "md" as const },
      { text: `Capital : ${w.capital.toLocaleString("fr-FR")} FCFA`, center: true, size: "sm" as const },
      { text: "\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501", center: true },
      { text: "STATUTS SNC", bold: true, center: true, size: "lg" as const },
      { text: "\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501", center: true },
      { text: `Associ\u00e9s : ${w.associes.length}`, spaceBefore: true },
  ...
  }, [w.denomination, w.capital, w.associes.length, w.gerant_nom, w.gerant_prenom, w.lieu_signature, w.date_signature]);

  return (
    <WizardLayout title="Statuts SNC" steps={STEPS} currentStep={w.currentStep}
      ...
      isLastDataStep={isLastDataStep} isDownloadStep={isDownloadStep} isGenerating={isGenerating} error={error} previewLines={previewLines}>
        ...
        {w.currentStep === 0 && (
          <>
            ...
            <Field colors={colors} label="D\u00e9nomination sociale" value={w.denomination} onChangeText={(v) => w.set({ denomination: v })} placeholder="Ex: ABC ET COMPAGNIE SNC" />
            <Field colors={colors} label="Objet social" value={w.objet_social} onChangeText={(v) => w.set({ objet_social: v })} multiline placeholder="Description de l'activit\u00e9..." />
        ...
        {w.currentStep === 1 && (
          <>
            <SectionTitle title={`Associ\u00e9s (${w.associes.length})`} colors={colors} />
            {w.associes.map((a, i) => (
              <View key={i} style={{ backgroundColor: colors.card, padding: 16, marginBottom: 12, borderWidth: 1, borderColor: colors.border }}>
                <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                  <Text style={{ fontFamily: fonts.semiBold, fontWeight: fontWeights.semiBold, fontSize: 15, color: colors.primary }}>Associ\u00e9 {i + 1}</Text>
                  {w.associes.length > 2 && (
                    <TouchableOpacity onPress={() => w.removeAssocie(i)}>
                      <Ionicons name="trash-outline" size={20} color={colors.danger || "#ef4444"} />
                    </TouchableOpacity>
                  )}
                </View>
                <Choice colors={colors} label="Civilit\u00e9" options={[{ value: "Monsieur", label: "M" }, { value: "Madame", label: "Mme" }]} value={a.civilite} onChange={(v) => w.updateAssocie(i, { civilite: v })} />
                ...
                <Field colors={colors} label="Date de naissance" value={a.date_naissance} onChangeText={(v) => w.updateAssocie(i, { date_naissance: v })} placeholder="Ex: 01/01/1980" />
                <Field colors={colors} label="Lieu de naissance" value={a.lieu_naissance} onChangeText={(v) => w.updateAssocie(i, { lieu_naissance: v })} />
                ...
                <Field colors={colors} label="Apport (FCFA)" value={a.apport ? String(a.apport) : ""} onChangeText={(v) => w.updateAssocie(i, { apport: parseInt(v) || 0 })} keyboardType="numeric" />
              </View>
            ))}
            <TouchableOpacity onPress={w.addAssocie} style={{ backgroundColor: colors.primary + "15", padding: 14, alignItems: "center", borderWidth: 1, borderColor: colors.primary, borderStyle: "dashed" }}>
          ...
          <>
            ...
            {w.capital > 0 && w.valeur_nominale > 0 && (
              <View style={{ backgroundColor: colors.card, padding: 16, borderWidth: 1, borderColor: colors.border }}>
                ...
                <Text style={{ fontFamily: fonts.medium, fontSize: 14, color: colors.text, marginTop: 4 }}>Total apports : {w.associes.reduce((s, a) => s + a.apport, 0).toLocaleString("fr-FR")} FCFA</Text>
              </View>
            )}
          </>
        )}
        ...
        {w.currentStep === 3 && (
          <>
            <SectionTitle title="G\u00e9rance" colors={colors} />
            ...
            <Field colors={colors} label="Date de naissance" value={w.gerant_date_naissance} onChangeText={(v) => w.set({ gerant_date_naissance: v })} placeholder="Ex: 01/01/1970" />
          ...
          return (
            <>
              <Section title="Soci\u00e9t\u00e9">
                ...
                <Row label="Forme" value="SNC" />
                <Row label="Si\u00e8ge" value={w.siege_social} />
                <Row label="Capital" value={`${w.capital.toLocaleString("fr-FR")} FCFA`} />
                <Row label="Dur\u00e9e" value={`${w.duree} ans`} />
              </Section>
              <Section title="Associ\u00e9s">
                {w.associes.map((a, i) => (
                  <Row key={i} label={`${a.civilite} ${a.prenom} ${a.nom}`} value={`${a.apport.toLocaleString("fr-FR")} FCFA`} />
                ))}
              </Section>
              <Section title="G\u00e9rance">
                <Row label="G\u00e9rant" value={`${w.gerant_civilite} ${w.gerant_prenom} ${w.gerant_nom}`} />
                <Row label="Dur\u00e9e mandat" value={w.gerant_duree_mandat} />
              </Section>
            </>
          );
        })()}
        ...
        {w.currentStep === 5 && (
          <>
            <View style={{ backgroundColor: "#ffffff", padding: 32, marginBottom: 20, borderWidth: 1, borderColor: colors.border }}>
              <Text style={{ fontFamily: fonts.heading, fontWeight: fontWeights.heading, fontSize: 20, color: "#1f2937", textAlign: "center", marginBottom: 16 }}>STATUTS SNC</Text>
              ...
              <Text style={{ fontFamily: fonts.regular, fontSize: 13, color: "#374151", textAlign: "center" }}>Soci\u00e9t\u00e9 en Nom Collectif au capital de {w.capital.toLocaleString("fr-FR")} FCFA</Text>
