...
interface SasuState {
  ...
  associe: SasuAssocieUnique;
  president: SasuPresident;
  has_cac: boolean;
  cac_titulaire: SasuCac;
  cac_suppleant: SasuCac;
  date_signature: string;
  lieu_signature: string;
  currentStep: number;
  set: (data: Partial<SasuState>) => void;
  setAssocie: (data: Partial<SasuAssocieUnique>) => void;
  setPresident: (data: Partial<SasuPresident>) => void;
  setCac: (type: "titulaire" | "suppleant", data: Partial<SasuCac>) => void;
  nextStep: () => void;
  prevStep: () => void;
  reset: () => void;
}

const defaultAssocie: SasuAssocieUnique = {
...
const defaultPresident: SasuPresident = {
  civilite: "Monsieur", nom: "", prenom: "", date_naissance: "",
  lieu_naissance: "", nationalite: "Congolaise", adresse: "", duree_mandat: "quatre",
};
const defaultCac: SasuCac = { civilite: "Monsieur", nom: "", prenom: "", adresse: "" };

const useSasuStore = create<SasuState>((set) => ({
  ...
  reset: () => set({
    ...
    capital: 1000000, valeur_nominale: 10000,
    mode_liberation: "intégralement", lieu_depot: "", nom_depositaire: "", date_certificat_depot: "",
    associe: { ...defaultAssocie }, president: { ...defaultPresident },
    has_cac: false, cac_titulaire: { ...defaultCac }, cac_suppleant: { ...defaultCac },
    date_signature: "", lieu_signature: "Brazzaville", currentStep: 0,
  }),
}));

const STEPS = ["Société", "Associé unique", "Capital", "Président", "Récapitulatif", "Aperçu"];
...
export default function SasuWizardScreen() {
  const { colors } = useTheme();
  const w = useSasuStore();
  const { isGenerating, generatedUrl, error, generate } = useDocumentGeneration("/generate/sasu", w.nextStep);
  const handleGenerate = () => generate({
        ...
        has_cac: w.has_cac,
        cac_titulaire: w.has_cac ? w.cac_titulaire : undefined,
        cac_suppleant: w.has_cac ? w.cac_suppleant : undefined,
  ...
  const previewLines = useMemo(() => {
    ...
    const lines: PreviewLine[] = [
      ...
      { text: "SAS Unipersonnelle", center: true, size: "md" as const },
      ...
      { text: "L'associé unique soussigné :", spaceBefore: true },
    ];
    const nom = w.associe.nom && w.associe.prenom ? `${w.associe.civilite} ${w.associe.prenom} ${w.associe.nom}` : "Associé unique (à compléter)";
    lines.push({ text: `- ${nom}${w.associe.adresse ? ", demeurant à " + w.associe.adresse : ""} ;` });
    lines.push(
      { text: "", spaceBefore: true },
      { text: "Article premier : Forme", bold: true, spaceBefore: true },
      { text: "Il est formé par l'associé unique une société par actions simplifiée unipersonnelle régie par l'Acte Uniforme OHADA." },
      { text: "", spaceBefore: true },
      ...
      { text: `Est nommé président : ${v(w.president.prenom)} ${v(w.president.nom)}, pour une durée de ${v(w.president.duree_mandat)}.` },
      { text: "", spaceBefore: true },
    ...
    return lines.filter(l => l.text !== undefined);
      ...
      w.capital, w.valeur_nominale, w.associe, w.president, w.lieu_signature, w.date_signature]);
  ...
  return (
    ...
    >
        ...
        {/* Étape 0 : Société */}
        {w.currentStep === 0 && (<>
          <Field colors={colors} label="Dénomination sociale" value={w.denomination} onChangeText={(v) => w.set({ denomination: v })} placeholder="Ex: ALPHA TECH" />
        ...
        {w.currentStep === 1 && (<>
          ...
          <View style={{ backgroundColor: "#fffbeb", padding: 12, borderLeftWidth: 3, borderLeftColor: colors.primary }}>
            <Text style={{ fontFamily: fonts.regular, fontSize: 13, color: "#92400e" }}>
              L'apport de l'associé unique doit correspondre au capital social. Capital min. SASU : 1.000.000 FCFA.
            </Text>
          </View>
        </>)}

        {/* Étape 2 : Capital + dépositaire */}
        {w.currentStep === 2 && (() => {
          const nbActions = w.valeur_nominale > 0 ? Math.floor(w.capital / w.valeur_nominale) : 0;
          return (<>
            ...
            <Text style={{ fontFamily: fonts.regular, fontSize: 13, color: colors.warning, marginBottom: 12 }}>Capital minimum SASU : 1.000.000 FCFA</Text>
            ...
            <View style={{ backgroundColor: "#ffffff", padding: 16, borderWidth: 1, borderColor: "#e2e8f0" }}>
              ...
              <View style={{ flexDirection: "row", justifyContent: "space-between", marginBottom: 8 }}>
                <Text style={{ fontFamily: fonts.regular, color: colors.textSecondary }}>Apport associé unique</Text>
        ...
        {w.currentStep === 3 && (<>
          ...
          <View style={{ marginBottom: 16 }}>
            <Text style={{ fontFamily: fonts.medium, fontSize: 14, color: colors.textSecondary, marginBottom: 8 }}>Remplir depuis l'associé unique :</Text>
            <TouchableOpacity onPress={() => w.setPresident({
          ...
          <Field colors={colors} label="Durée du mandat (en années)" value={w.president.duree_mandat} onChangeText={(v) => w.setPresident({ duree_mandat: v })} />
          <View style={{ backgroundColor: "#fffbeb", padding: 12, marginTop: 8, borderLeftWidth: 3, borderLeftColor: colors.primary }}>
            <Text style={{ fontFamily: fonts.regular, fontSize: 13, color: "#92400e" }}>
              Le président peut être l'associé unique ou un tiers (personne physique ou morale).
            </Text>
          </View>

          <SectionTitle title="Commissaire aux comptes (optionnel)" colors={colors} />
          <Choice colors={colors} label="Désigner un CAC ?" options={[
            { value: "non", label: "Non" },
            { value: "oui", label: "Oui" },
          ]} value={w.has_cac ? "oui" : "non"} onChange={(v) => w.set({ has_cac: v === "oui" })} />
          <View style={{ backgroundColor: "#f0f9ff", padding: 12, borderLeftWidth: 3, borderLeftColor: "#3b82f6", marginBottom: 12 }}>
            <Text style={{ fontFamily: fonts.regular, fontSize: 12, color: "#1e40af" }}>
              Obligatoire si : bilan {">"} 125M, CA {">"} 250M, effectif {">"} 50, ou si filiale/société mère.
            </Text>
          </View>
          {w.has_cac && (<>
        ...
        {w.currentStep === 4 && (() => {
          ...
          return (<>
            ...
            <Section title="Président">
              <Row label="Nom" value={`${w.president.prenom} ${w.president.nom}`} />
              <Row label="Mandat" value={`${w.president.duree_mandat} ans`} />
            </Section>
            {w.has_cac && (
        ...
        {w.currentStep === 5 && (<>
          <View style={{ alignItems: "center", paddingVertical: 32 }}>
            {generatedUrl ? (<>
              ...
              <Text style={{ fontFamily: fonts.regular, fontSize: 15, color: colors.textSecondary, marginTop: 8 }}>Statuts SASU — {w.denomination}</Text>
