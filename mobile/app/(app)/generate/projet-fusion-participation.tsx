...
interface ProjetFusionParticipationState {
  ...
  currentStep: number;
  set: (data: Partial<ProjetFusionParticipationState>) => void;
  nextStep: () => void;
  prevStep: () => void;
  reset: () => void;
}
...
const INITIAL: Omit<ProjetFusionParticipationState, "set" | "nextStep" | "prevStep" | "reset"> = {
...
const useStore = create<ProjetFusionParticipationState>((set) => ({
  ...INITIAL,
...
export default function ProjetFusionParticipationWizardScreen() {
  const { colors } = useTheme();
  const w = useStore();
  const { isGenerating, generatedUrl, error, generate } = useDocumentGeneration("/generate/projet-fusion-participation", w.nextStep);
  const handleGenerate = () => generate({
        ...
        actions_detenues_par_b: w.actions_detenues_par_b,
  ...
  const previewLines = useMemo(() => {
    const v = (s: string) => s || "...";
    const lines: any[] = [
      { text: "PROJET DE FUSION AVEC PARTICIPATION", bold: true, center: true, size: "xl" as const, spaceBefore: true },
      { text: "(Fusion-renonciation)", center: true, size: "md" as const },
      { text: "\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501", center: true },
      ...
      { text: "", spaceBefore: true },
      { text: `Participation : B d\u00e9tient ${w.actions_detenues_par_b || "..."} actions (${v(w.pourcentage_participation)}%)`, bold: true },
    ...
    return lines.filter((l: any) => l.text !== undefined);
      ...
      w.actions_detenues_par_b, w.pourcentage_participation, w.date_effet_fusion,
      w.actif_net_apporte, w.lieu_signature, w.date_signature]);

  return (
    <WizardLayout
      title="Projet de fusion avec participation"
      steps={STEPS}
    ...
    >
        ...
        {w.currentStep === 2 && (
          <>
            <SectionTitle title="Lien de capital (participation de B dans A)" colors={colors} />
            <Field colors={colors} label="Nombre d'actions de A d\u00e9tenues par B" value={w.actions_detenues_par_b ? String(w.actions_detenues_par_b) : ""} onChangeText={(v) => w.set({ actions_detenues_par_b: parseInt(v) || 0 })} keyboardType="numeric" />
            <Field colors={colors} label="Pourcentage de participation (%)" value={w.pourcentage_participation} onChangeText={(v) => w.set({ pourcentage_participation: v })} placeholder="Ex: 30" />
          </>
        )}
            ...
            <>
              <View style={{ backgroundColor: "#ffffff", padding: 32, marginBottom: 20, borderWidth: 1, borderColor: colors.border }}>
                <Text style={{ fontFamily: fonts.heading, fontWeight: fontWeights.heading, fontSize: 20, color: "#1f2937", textAlign: "center", marginBottom: 16 }}>
                  PROJET DE FUSION AVEC PARTICIPATION
                </Text>
                ...
                <Text style={{ fontFamily: fonts.regular, fontSize: 13, color: "#374151" }}>B d\u00e9tient {w.actions_detenues_par_b} actions ({w.pourcentage_participation}%)</Text>
