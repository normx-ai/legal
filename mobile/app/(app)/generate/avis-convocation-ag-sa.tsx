...
interface AvisConvocationAgSaState {
  denomination: string;
  siege_social: string;
  capital: number;
  type_ag: string;
  ...
  currentStep: number;
  set: (data: Partial<AvisConvocationAgSaState>) => void;
  nextStep: () => void;
  prevStep: () => void;
  reset: () => void;
}
...
const useAvisConvocationAgSaStore = create<AvisConvocationAgSaState>((set) => ({
  denomination: "",
  siege_social: "",
  capital: 0,
  type_ag: "ordinaire",
...
const STEPS = ["Société", "Assemblée", "Aperçu"];

// ── Main Screen ──

export default function AvisConvocationAgSaWizardScreen() {
  const { colors } = useTheme();
  const w = useAvisConvocationAgSaStore();
  const { isGenerating, generatedUrl, error, generate } = useDocumentGeneration("/generate/avis-convocation-ag-sa", w.nextStep);
  ...
  const previewLines = useMemo(() => {
    ...
    const lines: PreviewLine[] = [
      ...
      { text: "━━━━━━━━━━━━━━━━━━━━━━", center: true },
      { text: `AVIS DE CONVOCATION`, bold: true, center: true, size: "lg" as const },
      { text: `Assemblée Générale ${typeAgLabel}`, bold: true, center: true, size: "md" as const },
      { text: "━━━━━━━━━━━━━━━━━━━━━━", center: true },
      { text: "", spaceBefore: true },
      { text: `Les actionnaires de la société ${v(w.denomination)} sont convoqués en Assemblée Générale ${typeAgLabel},`, spaceBefore: true },
      { text: "", spaceBefore: true },
      { text: `Date : ${v(w.date_ag)}`, spaceBefore: true },
  ...
  }, [w.denomination, w.siege_social, w.capital, w.type_ag, w.date_ag, w.heure_ag,
      w.lieu_ag, w.ordre_du_jour, w.signataire_fonction, w.lieu_signature, w.date_signature]);

  return (
    <WizardLayout
      title="Avis de convocation AG (SA)"
      steps={STEPS}
        ...
        {w.currentStep === 1 && (() => {
          ...
          return (
            <>
              ...
              <Choice colors={colors} label="Signataire" options={[
              ...
              <Section title="Assemblée">
                ...
                <Row label="Signataire" value={w.signataire_fonction} />
              </Section>
            </>
          );
        })()}
            ...
            <>
              <View style={{ backgroundColor: "#ffffff", padding: 32, marginBottom: 20, borderWidth: 1, borderColor: colors.border }}>
                ...
                <Text style={{ fontFamily: fonts.regular, fontSize: 13, color: "#374151", marginBottom: 8 }}>
                  Les actionnaires de la société {w.denomination} sont convoqués en Assemblée Générale {typeAgLabel}, le {w.date_ag} à {w.heure_ag}, à {w.lieu_ag}.
                </Text>
