...
interface PvCarenceAgeState {
  ...
  currentStep: number;
  set: (data: Partial<PvCarenceAgeState>) => void;
  nextStep: () => void;
  prevStep: () => void;
  reset: () => void;
}
...
const useStore = create<PvCarenceAgeState>((set) => ({
  denomination: "",
  forme_juridique: "SA",
  siege_social: "",
  capital: "",
...
export default function PvCarenceAgeWizardScreen() {
  const { colors } = useTheme();
  const w = useStore();
  const { isGenerating, generatedUrl, error, generate } = useDocumentGeneration("/generate/pv-carence-age", w.nextStep);
  ...
  const previewLines = useMemo(() => {
    ...
    const lines: PreviewLine[] = [
      ...
      { text: "Assemblée Générale Extraordinaire", center: true, size: "md" as const },
      { text: "━━━━━━━━━━━━━━━━━━━━━━", center: true },
      { text: "", spaceBefore: true },
      { text: `L'an deux mille vingt-six, le ${v(w.date_ag)}, à ${v(w.heure_ag)}, les actionnaires de la société ${v(w.denomination)} ont été convoqués en Assemblée Générale Extraordinaire au ${v(w.lieu_ag)}.`, spaceBefore: true },
      { text: "", spaceBefore: true },
    ...
    >
        ...
        {w.currentStep === 1 && (
          <>
            ...
            <Field colors={colors} label="Quorum requis" value={w.quorum_requis} onChangeText={(v) => w.set({ quorum_requis: v })} placeholder="Ex: la moitié" />
          </>
        )}
          ...
          <>
            <View style={{ backgroundColor: "#ffffff", padding: 32, marginBottom: 20, borderWidth: 1, borderColor: colors.border }}>
              ...
              <Text style={{ fontFamily: fonts.regular, fontSize: 13, color: "#374151", marginBottom: 8 }}>
                AGE convoquée le {w.date_ag} à {w.heure_ag} au {w.lieu_ag}. Actions présentes : {w.actions_presentes} / {w.total_actions}. Quorum requis : {w.quorum_requis}. Le quorum n'étant pas atteint, l'assemblée ne peut délibérer.
              </Text>
              <Text style={{ fontFamily: fonts.regular, fontSize: 13, color: "#374151", marginBottom: 8 }}>
