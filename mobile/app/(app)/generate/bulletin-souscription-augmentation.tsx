...
interface BulletinAugmentationState {
  denomination: string;
  forme_juridique: string;
  siege_social: string;
  capital_avant: string;
  capital_apres: string;
  rccm: string;
  ...
  currentStep: number;
  set: (data: Partial<BulletinAugmentationState>) => void;
  nextStep: () => void;
  prevStep: () => void;
  reset: () => void;
}
...
const useStore = create<BulletinAugmentationState>((set) => ({
  denomination: "",
  forme_juridique: "SA",
  siege_social: "",
  capital_avant: "",
...
export default function BulletinSouscriptionAugmentationWizardScreen() {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      const { data } = await documentsApi.generate("/generate/bulletin-souscription-augmentation", {
        ...
        capital_avant: w.capital_avant,
        capital_apres: w.capital_apres,
        rccm: w.rccm,
        ...
        prime_emission: w.prime_emission,
  ...
  const previewLines = useMemo(() => {
    ...
    const lines: PreviewLine[] = [
      ...
      { text: `${v(w.forme_juridique)} au capital de ${v(w.capital_avant)} FCFA`, center: true, size: "md" as const },
      ...
      { text: "(Augmentation de capital)", center: true, size: "md" as const },
      { text: "━━━━━━━━━━━━━━━━━━━━━━", center: true },
    ...
    return lines.filter(l => l.text !== undefined);
  }, [w.denomination, w.forme_juridique, w.siege_social, w.capital_avant,
      ...
      w.mode_liberation, w.lieu_signature, w.date_signature]);

  return (
    <WizardLayout
      title="Bulletin de souscription (augmentation)"
      steps={STEPS}
    ...
    >
        ...
        {w.currentStep === 0 && (
          <>
            ...
            <Field colors={colors} label="Capital avant augmentation (FCFA)" value={w.capital_avant} onChangeText={(v) => w.set({ capital_avant: v })} placeholder="Ex: 10 000 000" />
            <Field colors={colors} label="Capital après augmentation (FCFA)" value={w.capital_apres} onChangeText={(v) => w.set({ capital_apres: v })} placeholder="Ex: 20 000 000" />
        ...
        {w.currentStep === 2 && (
          <>
            ...
            <Field colors={colors} label="Prime d'émission (FCFA)" value={w.prime_emission} onChangeText={(v) => w.set({ prime_emission: v })} placeholder="Ex: 5 000" />
            <Field colors={colors} label="Montant total versé (FCFA)" value={w.montant_verse} onChangeText={(v) => w.set({ montant_verse: v })} placeholder="Ex: 1 500 000" />
          ...
          <>
            <View style={{ backgroundColor: "#ffffff", padding: 32, marginBottom: 20, borderWidth: 1, borderColor: colors.border }}>
              ...
              <Text style={{ fontFamily: fonts.regular, fontSize: 13, color: "#374151", textAlign: "center", marginBottom: 20 }}>
                {w.forme_juridique} - Capital avant : {w.capital_avant} FCFA / Après : {w.capital_apres} FCFA
              </Text>
              ...
              <Text style={{ fontFamily: fonts.regular, fontSize: 13, color: "#374151", marginBottom: 8 }}>
                Montant versé : {w.montant_verse} FCFA - Mode de libération : {w.mode_liberation}
              </Text>
