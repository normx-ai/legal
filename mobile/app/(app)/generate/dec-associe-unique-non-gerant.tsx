...
interface DecAssoUniqueNonGerantState {
  denomination: string;
  siege_social: string;
  capital: string;
  valeur_nominale: number;
  associe_civilite: string;
  associe_nom: string;
  associe_prenom: string;
  ...
  nouveau_gerant_nom: string;
  nouveau_gerant_prenom: string;
  lieu_signature: string;
  date_signature: string;
  currentStep: number;
  set: (data: Partial<DecAssoUniqueNonGerantState>) => void;
...
const INITIAL: Omit<DecAssoUniqueNonGerantState, "set" | "toggleDecision" | "nextStep" | "prevStep" | "reset"> = {
  denomination: "",
  siege_social: "",
  capital: "",
  valeur_nominale: 0,
  associe_civilite: "Monsieur",
  associe_nom: "",
  associe_prenom: "",
  ...
  nouveau_gerant_nom: "",
  nouveau_gerant_prenom: "",
  lieu_signature: "Brazzaville",
  date_signature: "",
  currentStep: 0,
};

const useStore = create<DecAssoUniqueNonGerantState>((set) => ({
  ...INITIAL,
...
const STEPS = ["Société", "Associé", "Décisions", "Détails", "Aperçu"];
...
const DECISIONS_LIST: { key: string; label: string }[] = [
  ...
  { key: "renouvellement_gerant", label: "Renouvellement du gérant" },
  { key: "revocation_gerant", label: "Révocation du gérant" },
  { key: "remplacement_gerant", label: "Remplacement du gérant" },
  { key: "remuneration_gerant", label: "Rémunération du gérant" },
...
export default function DecAssoUniqueNonGerantWizardScreen() {
  const { colors } = useTheme();
  const w = useStore();
  const { isGenerating, generatedUrl, error, generate } = useDocumentGeneration("/generate/dec-associe-unique-non-gerant", w.nextStep);
  ...
  const handleGenerate = () => generate({
        ...
        associe_civilite: w.associe_civilite,
        associe_nom: w.associe_nom,
        associe_prenom: w.associe_prenom,
        gerant_civilite: w.gerant_civilite,
        gerant_nom: w.gerant_nom,
        gerant_prenom: w.gerant_prenom,
        date_envoi_documents: w.date_envoi_documents,
        ...
        duree_gerant: has("renouvellement_gerant") ? w.duree_gerant : undefined,
        remuneration_gerant: has("remuneration_gerant") ? w.remuneration_gerant : undefined,
        ...
        nouveau_gerant_nom: has("remplacement_gerant") ? w.nouveau_gerant_nom : undefined,
        nouveau_gerant_prenom: has("remplacement_gerant") ? w.nouveau_gerant_prenom : undefined,
  ...
  const previewLines = useMemo(() => {
    ...
    const lines: PreviewLine[] = [
      ...
      { text: "━━━━━━━━━━━━━━━━━━━━━━", center: true },
      { text: "DÉCISIONS DE L'ASSOCIÉ UNIQUE NON GÉRANT", bold: true, center: true, size: "lg" as const },
      { text: "━━━━━━━━━━━━━━━━━━━━━━", center: true },
      { text: "", spaceBefore: true },
      { text: `${v(w.associe_civilite)} ${v(w.associe_prenom)} ${v(w.associe_nom)}, associé unique de la société ${v(w.denomination)}, dont le gérant est ${v(w.gerant_civilite)} ${v(w.gerant_prenom)} ${v(w.gerant_nom)}, a pris les décisions suivantes le ${v(w.date_decisions)} à ${v(w.heure_decisions)}.`, spaceBefore: true },
      { text: "", spaceBefore: true },
  ...
  }, [w.denomination, w.siege_social, w.capital, w.associe_civilite, w.associe_nom, w.associe_prenom,
      w.gerant_civilite, w.gerant_nom, w.gerant_prenom, w.date_decisions, w.heure_decisions,
      w.decisions_selectionnees, w.lieu_signature, w.date_signature]);

  return (
    <WizardLayout
      title="Décisions associé unique non gérant (SARL)"
      steps={STEPS}
    ...
    >
        ...
        {w.currentStep === 1 && (
          <>
            <SectionTitle title="Associé unique" colors={colors} />
            <Choice colors={colors} label="Civilité" options={[{ value: "Monsieur", label: "Monsieur" }, { value: "Madame", label: "Madame" }]} value={w.associe_civilite} onChange={(v) => w.set({ associe_civilite: v })} />
            <View style={{ flexDirection: "row", gap: 8 }}>
              <View style={{ flex: 1 }}><Field colors={colors} label="Nom" value={w.associe_nom} onChangeText={(v) => w.set({ associe_nom: v })} /></View>
              <View style={{ flex: 1 }}><Field colors={colors} label="Prénom" value={w.associe_prenom} onChangeText={(v) => w.set({ associe_prenom: v })} /></View>
            </View>
            ...
            <Field colors={colors} label="Date d'envoi des documents" value={w.date_envoi_documents} onChangeText={(v) => w.set({ date_envoi_documents: v })} placeholder="Ex: 1er mars 2026" />

            <SectionTitle title="Date et heure des décisions" colors={colors} />
        ...
        {w.currentStep === 2 && (
          <>
            <Text style={{ fontFamily: fonts.regular, fontSize: 13, color: colors.textSecondary, marginBottom: 16 }}>
              Sélectionnez les décisions prises par l'associé unique non gérant.
            </Text>
        ...
        {w.currentStep === 3 && (
          <>
            ...
            {has("renouvellement_gerant") && (
              <>
                <SectionTitle title="Renouvellement du gérant" colors={colors} />
                <Field colors={colors} label="Durée du mandat" value={w.duree_gerant} onChangeText={(v) => w.set({ duree_gerant: v })} placeholder="Ex: 4 ans" />
              </>
            )}

            {has("remplacement_gerant") && (
              <>
                <SectionTitle title="Remplacement du gérant" colors={colors} />
                <Field colors={colors} label="Nom du nouveau gérant" value={w.nouveau_gerant_nom} onChangeText={(v) => w.set({ nouveau_gerant_nom: v })} />
                <Field colors={colors} label="Prénom du nouveau gérant" value={w.nouveau_gerant_prenom} onChangeText={(v) => w.set({ nouveau_gerant_prenom: v })} />
              </>
            )}

            {has("remuneration_gerant") && (
              <>
                <SectionTitle title="Rémunération du gérant" colors={colors} />
                <Field colors={colors} label="Détails de la rémunération" value={w.remuneration_gerant} onChangeText={(v) => w.set({ remuneration_gerant: v })} placeholder="Décrire la rémunération..." multiline />
              </>
            )}
            ...
            <>
              <View style={{ backgroundColor: "#ffffff", padding: 32, marginBottom: 20, borderWidth: 1, borderColor: colors.border }}>
                <Text style={{ fontFamily: fonts.heading, fontWeight: fontWeights.heading, fontSize: 20, color: "#1f2937", textAlign: "center", marginBottom: 16 }}>
                  DÉCISIONS DE L'ASSOCIÉ UNIQUE NON GÉRANT
                </Text>
                ...
                <Text style={{ fontFamily: fonts.regular, fontSize: 13, color: "#374151", marginBottom: 8 }}>
                  {w.associe_civilite} {w.associe_prenom} {w.associe_nom}, associé unique de la société {w.denomination}, dont le gérant est {w.gerant_civilite} {w.gerant_prenom} {w.gerant_nom}, a pris les décisions suivantes le {w.date_decisions} à {w.heure_decisions}.
                </Text>
