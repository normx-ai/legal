...
interface SaAgState {
  ...
  associes: { civilite: string; nom: string; prenom: string; date_naissance: string; lieu_naissance: string; nationalite: string; profession: string; adresse: string; apport: number; type_apport: string }[];
  ag: { civilite: string; nom: string; prenom: string; date_naissance: string; lieu_naissance: string; nationalite: string; adresse: string; duree_mandat: string };
  cac_titulaire: { civilite: string; nom: string; prenom: string; adresse: string };
  cac_suppleant: { civilite: string; nom: string; prenom: string; adresse: string };
  clause_cession: string; delai_preemption: string; duree_inalienabilite: string;
  representation: string;
  date_signature: string; lieu_signature: string;
  currentStep: number;
  set: (data: Partial<SaAgState>) => void;
  setAssocie: (i: number, data: Partial<SaAgState["associes"][0]>) => void;
  addAssocie: () => void;
  removeAssocie: (i: number) => void;
  setAg: (data: Partial<SaAgState["ag"]>) => void;
  setCac: (type: "titulaire" | "suppleant", data: Partial<SaAgState["cac_titulaire"]>) => void;
  nextStep: () => void; prevStep: () => void; reset: () => void;
}

const defaultAssocie = { civilite: "Monsieur", nom: "", prenom: "", date_naissance: "", lieu_naissance: "", nationalite: "Congolaise", profession: "", adresse: "", apport: 0, type_apport: "numeraire" };
const defaultAg = { civilite: "Monsieur", nom: "", prenom: "", date_naissance: "", lieu_naissance: "", nationalite: "Congolaise", adresse: "", duree_mandat: "deux ans" };
const defaultCac = { civilite: "Monsieur", nom: "", prenom: "", adresse: "" };

const useSaAgStore = create<SaAgState>((set) => ({
  ...
  ag: { ...defaultAg }, cac_titulaire: { ...defaultCac }, cac_suppleant: { ...defaultCac },
  clause_cession: "agrement", delai_preemption: "3", duree_inalienabilite: "5",
  representation: "libre",
  ...
  reset: () => set({ denomination: "", sigle: "", objet_social: "", siege_social: "", ville: "Brazzaville", pays: "République du Congo", duree: 99, exercice_debut: "1er janvier", exercice_fin: "31 décembre", premier_exercice_fin: "", capital: 10000000, valeur_nominale: 10000, mode_liberation: "intégralement", lieu_depot: "", nom_depositaire: "", date_certificat_depot: "", associes: [{ ...defaultAssocie }], ag: { ...defaultAg }, cac_titulaire: { ...defaultCac }, cac_suppleant: { ...defaultCac }, clause_cession: "agrement", delai_preemption: "3", duree_inalienabilite: "5", representation: "libre", date_signature: "", lieu_signature: "Brazzaville", currentStep: 0 }),
}));

const STEPS = ["Société", "Actionnaires", "Capital", "Admin. Général", "CAC", "Clauses", "Récapitulatif", "Aperçu"];
...
export default function SaAgWizardScreen() {
  const { colors } = useTheme();
  const w = useSaAgStore();
  const { isGenerating, generatedUrl, error, generate } = useDocumentGeneration("/generate/sa-ag", w.nextStep);
  const handleGenerate = () => generate({
        ...
        associes: w.associes, ag: w.ag, cac_titulaire: w.cac_titulaire, cac_suppleant: w.cac_suppleant,
        clause_agrement: w.clause_cession === "agrement", clause_preemption: w.clause_cession === "preemption", clause_inalienabilite: w.clause_cession === "inalienabilite",
        delai_preemption: w.delai_preemption, duree_inalienabilite: w.duree_inalienabilite,
        representation_libre: w.representation === "libre", representation_actionnaire: w.representation === "actionnaire",
  ...
  const previewLines = useMemo(() => {
    ...
    const lines: PreviewLine[] = [
      ...
      { text: "SA avec Administrateur Général", center: true, size: "md" as const },
    ...
    lines.push(
      ...
      { text: "Il est formé entre les soussignés une société anonyme avec administrateur général régie par l'Acte Uniforme OHADA." },
      { text: "", spaceBefore: true },
    ...
    return lines.filter(l => l.text !== undefined);
      ...
      w.capital, w.valeur_nominale, w.associes, w.ag, w.lieu_signature, w.date_signature]);

  return (
    <WizardLayout
      title="Statuts SA — Administrateur Général"
      steps={STEPS}
    ...
    >
        ...
        {w.currentStep === 3 && (<>
          ...
          {w.associes.length > 0 && (
            <View style={{ marginBottom: 16 }}>
              <Text style={{ fontFamily: fonts.medium, fontSize: 14, color: colors.textSecondary, marginBottom: 8 }}>Remplir depuis un actionnaire :</Text>
              <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 8 }}>
                {w.associes.map((a, i) => (
                  <TouchableOpacity key={i} onPress={() => w.setAg({ civilite: a.civilite, nom: a.nom, prenom: a.prenom, date_naissance: a.date_naissance, lieu_naissance: a.lieu_naissance, nationalite: a.nationalite, adresse: a.adresse })}
        ...
        {/* Étape 5 : Clauses */}
        {w.currentStep === 5 && (<>
          <SectionTitle title="Cession d'actions à des tiers (Art. 13)" colors={colors} />
          <Choice colors={colors} label="Clause applicable" options={[
            { value: "agrement", label: "Agrément" },
            { value: "preemption", label: "Préemption" },
            { value: "inalienabilite", label: "Inaliénabilité" },
          ]} value={w.clause_cession} onChange={(v) => w.set({ clause_cession: v })} />
          {w.clause_cession === "preemption" && <Field colors={colors} label="Délai de préemption (mois)" value={w.delai_preemption} onChangeText={(v) => w.set({ delai_preemption: v })} />}
          {w.clause_cession === "inalienabilite" && <Field colors={colors} label="Durée d'inaliénabilité (années, max 10)" value={w.duree_inalienabilite} onChangeText={(v) => w.set({ duree_inalienabilite: v })} />}

          <SectionTitle title="Représentation en assemblée (Art. 16)" colors={colors} />
          <Choice colors={colors} label="Un actionnaire peut se faire représenter par" options={[
            { value: "libre", label: "Toute personne" },
            { value: "actionnaire", label: "Un actionnaire uniquement" },
          ]} value={w.representation} onChange={(v) => w.set({ representation: v })} />
        </>)}
        ...
        {w.currentStep === 6 && (() => {
          ...
          return (<>
            <Section title="Société">
              ...
              <Row label="Forme" value="SA avec Administrateur Général" />
            ...
            <Section title="Capital">
              ...
              <Row label="Actions" value={`${nbActions} × ${w.valeur_nominale.toLocaleString("fr-FR")} FCFA`} />
            ...
            <Section title="Clauses">
              <Row label="Cession tiers" value={w.clause_cession === "agrement" ? "Agrément" : w.clause_cession === "preemption" ? "Préemption" : "Inaliénabilité"} />
              <Row label="Représentation" value={w.representation === "libre" ? "Libre" : "Actionnaires uniquement"} />
            </Section>
        ...
        {w.currentStep === 7 && (<>
          <View style={{ alignItems: "center", paddingVertical: 32 }}>
            {generatedUrl ? (<>
              ...
              <Text style={{ fontFamily: fonts.regular, fontSize: 15, color: colors.textSecondary, marginTop: 8 }}>Statuts SA AG — {w.denomination}</Text>
