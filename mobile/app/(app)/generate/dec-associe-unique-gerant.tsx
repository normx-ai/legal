...
interface DecAssoUniqueGerantState {
  denomination: string;
  siege_social: string;
  capital: string;
  ...
  currentStep: number;
  set: (data: Partial<DecAssoUniqueGerantState>) => void;
...
const INITIAL: Omit<DecAssoUniqueGerantState, "set" | "toggleDecision" | "nextStep" | "prevStep" | "reset"> = {
  denomination: "",
  siege_social: "",
  capital: "",
  valeur_nominale: 0,
...
const useStore = create<DecAssoUniqueGerantState>((set) => ({
  ...INITIAL,
...
export default function DecAssoUniqueGerantWizardScreen() {
  const { colors } = useTheme();
  const w = useStore();
  const { isGenerating, generatedUrl, error, generate } = useDocumentGeneration("/generate/dec-associe-unique-gerant", w.nextStep);
  ...
  const previewLines = useMemo(() => {
    ...
    const lines: PreviewLine[] = [
      ...
      { text: "━━━━━━━━━━━━━━━━━━━━━━", center: true },
      { text: "DÉCISIONS DE L'ASSOCIÉ UNIQUE GÉRANT", bold: true, center: true, size: "lg" as const },
      { text: "━━━━━━━━━━━━━━━━━━━━━━", center: true },
      { text: "", spaceBefore: true },
      { text: `${v(w.associe_civilite)} ${v(w.associe_prenom)} ${v(w.associe_nom)}, associé unique et gérant de la société ${v(w.denomination)}, a pris les décisions suivantes le ${v(w.date_decisions)} à ${v(w.heure_decisions)}.`, spaceBefore: true },
      { text: "", spaceBefore: true },
    ...
    return lines.filter(l => l.text !== undefined);
      ...
      w.date_decisions, w.heure_decisions, w.decisions_selectionnees, w.lieu_signature, w.date_signature]);

  return (
    <WizardLayout
      title="Décisions associé unique gérant (SARL)"
      steps={STEPS}
        ...
        {w.currentStep === 2 && (
          <>
            <Text style={{ fontFamily: fonts.regular, fontSize: 13, color: colors.textSecondary, marginBottom: 16 }}>
              Sélectionnez les décisions prises par l'associé unique gérant.
            </Text>
            ...
            <>
              <View style={{ backgroundColor: "#ffffff", padding: 32, marginBottom: 20, borderWidth: 1, borderColor: colors.border }}>
                ...
                <Text style={{ fontFamily: fonts.regular, fontSize: 13, color: "#374151", marginBottom: 8 }}>
                  {w.associe_civilite} {w.associe_prenom} {w.associe_nom}, associé unique et gérant de la société {w.denomination}, a pris les décisions suivantes le {w.date_decisions} à {w.heure_decisions}.
                </Text>
