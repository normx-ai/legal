...
export default function ConvAgeWizardScreen() {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    setError("");
    try {
      const { data } = await documentsApi.generate("/generate/conv-age", {
      ...
      {w.currentStep === 4 && (
        <>
          <View style={{ backgroundColor: "#ffffff", padding: 32, marginBottom: 20, borderWidth: 1, borderColor: colors.border }}>
            <Text style={{ fontFamily: fonts.heading, fontWeight: fontWeights.heading, fontSize: 20, color: "#1f2937", textAlign: "center", marginBottom: 16 }}>CONVOCATION AGE</Text>
